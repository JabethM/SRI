...
def run_script(args):
    subprocess.run(["python", "-m", "src.data_extraction.DataCollector_Deterministic", str(args[0]), str(args[1]), str(args[2])])


def main(path, variable1, variable2, batch, name_suffix):
    arguments = [(os.path.join(path, "inputs", f'{variable1}_{variable2}.{batch}_{b}.json'), path, str(name_suffix) + '.' + str(batch) + '.' + str(b)) for b in range(10)]
